...
class BalanceSystem {
  ...
  claimBonus() {
    ...
    const newbal = (this.balance += this.bonusAmount);
  ...
  updateDisplay() {
    ...
    if (balanceElement) {
      balanceElement.textContent = "$" + newbal.toFixed(2);
    }
  }
...
class SlotsGame {
  ...
  spin() {
    ...
    console.log(
      "Начало вращения. Ставка:",
      this.betAmount,
      "Баланс:",
      this.balanceSystem.getBalance(),
    );

    if (!this.balanceSystem.subtractMoney(this.betAmount)) {
      this.balanceSystem.showMessage(
        "Недостаточно денег! Нужно $" + this.betAmount.toFixed(2),
      );
      return;
    }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (winAmount > 0) {
          ...
          this.balanceSystem.showMessage(
            "Ты выиграл $" + winAmount.toFixed(2) + "!",
            "success",
          );
